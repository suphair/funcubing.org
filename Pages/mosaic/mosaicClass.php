...
class Mosaic {
    ...
    public static function Init() {
        ...
        self::$cubes = $_SESSION['cubes'] ?? self::CUBE_AMOUNT;
        self::$images = $_SESSION['images'] ?? [];
        self::$pixels = $_SESSION['pixels'] ?? 3;
        self::$color = $_SESSION['color'] ?? 'B';
        self::$widthCubes_pdf = $_SESSION['widthCubes_pdf'] ?? 3;
        self::$heightCubes_pdf = $_SESSION['heightCubes_pdf'] ?? 4;
        self::$step_name = $_SESSION['step_name'] ?? self::StepPreparation;
        self::$pdfImages = $_SESSION['pdfImages'] ?? reset(self::$pdfImagesVars);
    ...
    public static function Reset($delete = true) {
        if ($delete) {
    ...
    private static function UpdateID($id) {
        $_SESSION['GUID'] = $id;
        self::$id = $id;
        if (!file_exists("Images")) {
            mkdir("Images");
        }
        if (!file_exists("Images/mosaic")) {
            mkdir("Images/mosaic");
        }
        if (!file_exists("Images/mosaic/" . self::$id)) {
            mkdir("Images/mosaic/" . self::$id);
        }
        self::$add_schema = false;
        self::$dirName = "Images/mosaic/" . self::$id . "/";
    ...
    public static function changeCube($amount = FALSE) {
        if ($amount) {
            self::setCubes($amount);
        }
    ...
    static private function setSchemas($code = FALSE) {
        if (self::$step == 1) {
    ...
    static public function generatePaints($code = false) {
        $ColorSchemas = self::setSchemas($code);
