...
<form method="POST" action="?ranking_rename_add">
    WCA ID 
    <input pattern="[0-9]{4}[A-Za-z]{4}[0-9]{2}" maxlength="10" title='NNNNXXXXNN' name='wcaid' autocomplete="off">
    <?= t('New name WCA', 'Новое WCA имя') ?> <input name="name" autocomplete="off"/>        
...
<table class='table thead_stable'>
    ...
    <tbody>
        ...
        <form method="POST" action="?ranking_rename_delete">
            <td><?= $row->wcaid ?></td>
            <td style="color:red"><?= explode(" (", $person->name)[0] ?></td>
