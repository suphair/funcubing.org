...
function GoalRecordFormat($Records,$event,$type){
    if(!isset($Records[$event][$type]))return '';
    $result=$Records[$event][$type];
...
function GoalProgress($Records,$event,$type,$goal) {
    if(!isset($Records[$event][$type]) or !isset($goal['Goal']) or !$goal['Goal']){
        return '';
    }
    $record=$Records[$event][$type];
    $goalInt=GoalResultToInt($goal['Goal']);
    if($goalInt>=$record) return '';
    return (round(($record-$goalInt)/$record*100,1)).'%';
}



function GoalResultToInt($result){
    if($result=='DNF' or $result=='DNS')return 0;
...
function GoalImageCreate($competition,$competitorWID){
    ...
    foreach(DataBaseClass::QueryGenerate() as $row){
        $Disciplines[$row['GoalDiscipline_Code']]=$row['GoalDiscipline_Name'];
          ...
          #-----------------
            $font="//Fonts/Stolzl-Book.otf";
            $font_b="//Fonts/Stolzl-Bold.otf";
            $font_i="//Fonts/Stolzl-Light.otf";
            ...
            $logo= imagecreatefrompng("Image/GC.png");
            ...
            $weith=GetParam(12,"//Fonts/Stolzl-Book.otf",'Goals')['weith'];                
            ...
            foreach($goals as $goal){ 
                if($goal['Discipline']!=$discipline){
                    $Y+=8;
                    $ImgIcon=imagecreatefrompng("Image/GoalImage/".$Disciplines[$goal['Discipline']].".png");
            ...
            $fileName="Image/GoalImages/".$competition."_".$competitorWID."_".md5("GOALS".$competition.$competitorWID).".png";
            if(sizeof($goals)){
