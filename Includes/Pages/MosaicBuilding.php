...
<link href="<?= PageIndex()?>style_mosaicbuilding.css" rel="stylesheet">
        <h1>Mosaic Bulding</h1>
        <h2>Current step: <?= Mosaic::$step_name; ?></h2>
        <div class="content_border"> 
            ...
            <div class="form">
                ...
                <form style="vertical-align: middle; float: left;" method="post" enctype="multipart/form-data" ID="Form_Reset" action="<?= PageIndex()?>Actions/MosaicBuildingReset">
                        <input class="delete" type="submit" value="Reset">
      ...
      <?php if(Mosaic::$step==1 and Mosaic::$step_name==Mosaic::StepChoosing){ ?>     
            <div class="content_border" id="custom_schema"  <?= Mosaic::$customColorsSchema==['_','_','_','_','_','_']?'style="display: none;"':'' ?>>
                ...
                <div class="custom_schema"  >
                    ...
                    <?php foreach(glob(Mosaic::$dirName."LayerL*.png") as $name){ 
                    ...
                    <div class="CustomSchema">
                        <img  src="<?= PageIndex()?><?=$name ?>?<?= time(); ?>"><p>
                            <?php 
                            $i=0;
                            foreach(Mosaic::$colors as $c=>$tmp){ $i++; if($i==4){?></p><p><?php }?>
...
<div class="content_border">
      <div class="Wrapper">  
            ...
            if(!Mosaic::$step){
                ...
                if(in_array('_',Mosaic::$customColorsSchema)){
                    $title="Substep ".Mosaic::$step." of ".(STEPS)." &#9642; Click on the image most similar to the original";
                }else{
                    $title="Substep ".Mosaic::$step." of ".(STEPS)." "
                            . " &#9642; Click on the image if the custom scheme is correct"
                            . " <br>&#9642; All right, <a target='_blank' href='".PageIndex()."Actions/MosaicBuildingPDF/".implode(Mosaic::$customColorsSchema)."'>generate PDF</a> with custom scheme!";
                }
            ...
            <table width=100% cellpadding='5'>
                <tr>
                    ...
                    <td valign="top" style="border-bottom:0px;">
                       <?php if(Mosaic::$step<=STEPS and Mosaic::$step){
                           if(Mosaic::$color=='W'){
                               $Color_1='W';$Color_2='B';
                           }else{
                               $Color_1='B';$Color_2='W';
                           } ?> 
                        <?php  foreach(glob(Mosaic::$dirNameStep."/*") as $i=>$name){ 
