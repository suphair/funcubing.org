...
<?php if (isset($Current_discipline)) { ?>       
    ...
    if (isset($current_discipline)) {
        ...
        <table class="table_new">
            ...
            <tr>
                ...
                <td style="vertical-align: middle;"><div style="text-align:left; width:100%; font-size:18px; padding:2px; color:var(--gray);" id="ParseName"></div>
                    <?php for ($i = 1; $i <= $Attemps; $i++) { ?>
                    <td>
                        <div style="text-align:center; width:100px; border:1px solid var(--white); padding:2px;" class="Attempt_edit" id="Attempt<?= $i ?>">
                        </div>
                    </td>
                <?php } ?>
                <?php foreach ($Formats as $f => $format) { ?>
            ...
            <?php foreach ($competitors as $competitor) { ?>
                ...
                <tr class="<?= $competitor ['MeetingCompetitorDiscipline_Place'] <= 3 ? 'podium' : '' ?>" onclick="
                    ...
                    ">
                    <td><?= $competitor['MeetingCompetitorDiscipline_Place'] ?></td>
                    <td><?= $competitor['MeetingCompetitor_Name'] ?></td>
                    <?php for ($i = 1; $i <= $Attemps; $i++) { ?>
                        <td class="attempt"> <?= $competitor['MeetingCompetitorDiscipline_Attempt' . $i] ?></td>
                    <?php } ?>
                    <?php foreach ($Formats as $f => $format) { ?>  
                        <td class="<?= $f == 0 ? 'border-left-solid' : '' ?> attempt"> <?= $competitor['MeetingCompetitorDiscipline_' . $format] ?></td>
                    <?php } ?>
                    <td>
    ...
    <?php if ($Current_discipline['MeetingDiscipline_Round'] == 1) { ?>
        ...
        <?php if (sizeof($competitors)) { ?>
            <h3>Adding competitors to the discipline</h3>
            <form method="POST" action="">
                ...
                <input type="submit" value="Add registrations" name="AddRegistrations">
            </form>
        <?php } ?>
    <?php } else { ?>
        <?php
...
order by MCD.Place");
        ...
        ?>
        <?php if (sizeof($competitors)) { ?>
            <h3>Adding competitors to the final</h3>
            <form method="POST" action="">
                <table>
                    <tr class="tr_title">
                        <td>Place</td>
                ...
                <input type="hidden" value="<?= $current_discipline ?>" name="Discipline">
                <input type="submit" value="Add competitions" name="AddRegistrations">
            </form>  
        <?php } ?>
    <?php } ?>
<?php } ?>
