...
<h2> <?=$competition['GoalCompetition_Name'] ?> &#9642; <?= date("d.m.Y",strtotime($competition['GoalCompetition_DateStart']))." - ". date("d.m.Y",strtotime($competition['GoalCompetition_DateEnd'])) ?></h2>
        <?php
        DataBaseClass::Query("Select * from Goal where Competition='$competition_wca' and  Competitor=".$Competitor->id." and Result is not null");
        $dateLoad=''; 
                ...
                if($status==200 and sizeof($results)){
                    ...
                    foreach(DataBaseClass::getRows() as $row){
                        if(isset($result_best[$row['Format']][$row['Discipline']])){
                            $result_in=ResultEnter($result_best[$row['Format']][$row['Discipline']],$row['Discipline'],$row['Format']);
    ...
    <?php
        ...
        <table>
            ...
            <?php foreach($GoalEvents['Events'] as $event){
                ...
                <tr>
                    ...
                    <td colspan="5" align="center">
                        <span class="error">Not implemented for 3x3x3 Multi-Blind</span>
                    </td>
                    <?php }else{ ?>
            ...
            if($TimeFixed=DataBaseClass::getRow()['TimeFixed']){ ?>
                <span class="error">Can be changed until <?= $TimeFixed; ?></span><br>
            <?php } ?>
